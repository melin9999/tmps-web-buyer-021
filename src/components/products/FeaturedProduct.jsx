...
const FeaturedProduct = ({itemWidth, val, vertical}) => {
  return (
    <>
      {vertical?
        <div className='flex flex-col justify-center items-center mb-10' style={{width: (itemWidth)}}>
          <div className="flex flex-row justify-center items-center w-[150px] h-[150px] relative mb-3">
          ...
          <div className="flex flex-col w-full justify-start items-start ml-3 mb-3">
            <span className='flex flex-col w-full text-xs font-semibold h-[35px] overflow-hidden' style={{borderBottom: '1px solid #D1D5DB'}}>{val.heading}</span>
        ...
        <div className="flex flex-row items-start" style={{width: itemWidth}}>
          <div className="flex flex-row justify-center items-center w-[150px] h-[150px] relative">
            {val.image_url==="none"?<CameraAlt sx={{width: 120, height: 120, color: '#cbd5e1'}}/>:
            <Image src={val.image_url} alt="product image" fill sizes={'150px'} priority={true} style={{objectFit: 'contain'}}/>}
          </div>
          <div className="flex flex-col justify-between items-start overflow-hidden"style={{width: (itemWidth-151), height: 150}}>
            <div className="flex flex-col w-full justify-start items-start ml-1">
              <span className='flex flex-col w-full text-xs font-semibold h-[35px] overflow-hidden' style={{borderBottom: '1px solid #D1D5DB'}}>{val.heading}</span>
              <span className='flex flex-col w-full text-[11px] font-normal break-words h-[50px] overflow-hidden mt-1'>{val.short_description}</span>
              <div className='flex flex-row overflow-hidden gap-1 flex-wrap mt-1 w-full h-[22px]'>
                {val.featured==="yes"&&<span className='flex flex-col justify-center items-center h-[22px] py-1 px-1 bg-yellow-200 text-yellow-800 rounded text-[10px] font-bold'>Featured</span>}
        ...
        <div className='flex flex-col justify-center items-center bg-zinc-200 pb-3' style={{width: (itemWidth)}}>
          <div className='flex flex-col justify-center items-center bg-white' style={{width: (itemWidth)}}>
            <div className='flex flex-col xxs:flex-row justify-between items-start'>
              <div className='flex justify-center items-center w-full xxs:w-[150px] mt-2 xxs:mt-0 mb-1 xxs:mb-0'>
                <div className='flex justify-center items-center relative' style={{width: (150), height: (150)}}>
                  {val.image_url==="none"?<CameraAlt sx={{width: 80, height: 80, color: '#cbd5e1'}}/>:
                  <Image src={val.image_url} alt="product image" fill sizes={'150px'} priority={true} style={{objectFit: 'contain'}}/>}
                </div>
              </div>
              <div className="flex flex-col justify-start items-start" style={{width: itemWidth>320?(itemWidth-150):itemWidth}}>
                ...
                <div className='flex flex-row overflow-hidden px-2 gap-1 flex-wrap h-[50px] xxs:h-[75px] xs:h-[65px] mb-2 xxs:mb-0'>
                ...
                <div className='hidden xs:flex flex-row justify-end items-end w-full py-1 px-2 pb-1'>
                  {parseFloat(val.discount)>0.0 ?
                    <div className='flex flex-col justify-center items-end w-[70px] xs:w-[110px] mr-3'>
                      <span className='text-[10px] font-bold text-red-400'>{val.discount+"% off"}</span>
                      <span className='text-[11px] font-semibold text-zinc-600 line-through'>{"Rs. "+parseFloat(val.price).toFixed(2)}</span>
                  ...
                  <div className='flex flex-row justify-between items-center px-2 py-1 w-[100px] xs:w-[130px] h-[30px]' style={{border: '1px solid #10B981'}}>
                    <span className='font-semibold text-[10px] xs:text-xs text-emerald-600'>{"Rs."}</span>
                    <span className='text-[11px] xs:text-xs font-semibold text-emerald-600'>{parseFloat(val.discounted_price).toFixed(2)}</span>
                  </div>
                </div>
            ...
            <div className='flex xs:hidden flex-row justify-end items-center w-full py-1 px-2 pb-1'>
              {parseFloat(val.discount)>0.0 ?
                <div className='flex flex-col justify-center items-end w-[70px] xs:w-[110px] mr-3'>
                  <span className='text-[10px] font-bold text-red-400'>{val.discount+"% off"}</span>
                  <span className='text-[11px] font-semibold text-zinc-600 line-through'>{"Rs. "+parseFloat(val.price).toFixed(2)}</span>
